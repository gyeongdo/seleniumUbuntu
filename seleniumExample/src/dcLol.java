...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//        String fmurl = "http://www.etoland.co.kr/bbs/board.php?bo_table=etohumor02&wr_id=1415084";
        String fmurl = "http://www.etoland.co.kr/bbs/board.php?bo_table=etohumor02&wr_id=1415951";
...
//        headers.put("User-Agent","Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/74.0.3729.169 Chrome/74.0.3729.169 Safari/537.36");
        ...
        try{
            ...
            Connection.Response response = Jsoup.connect(fmurl)
                    .headers(headers)
                    .timeout(20000)
                    .execute();
            ...
            Element divHtml =  doc.select("#view11_content").first();

            // p tag or img
            if( !divHtml.select("p").isEmpty() ) {
                for(Element childDiv : divHtml.select("p")){
                    //img 일때
                    if( !childDiv.select("img").isEmpty() ){

                        String src = childDiv.select("img").attr("src");
                        System.out.println("src : "+ src);
                        html_tag.add(etorrentUrl+src);

                    } else {
                        html_tag.add(childDiv.toString());
                    }
                }
            } else if( !divHtml.select("img").isEmpty() ) {
                for(Element childDiv : divHtml.select("img")){
                    //img 일때
                    if( !childDiv.select("img").isEmpty() ){
...
//        }
    ...
    private static String downloadImage(String strImageURL){
        ...
        String reStrImageURL = strImageURL;
        int index = reStrImageURL.lastIndexOf(".");
        reStrImageURL = reStrImageURL.substring(index);

        String sliceStrImgUrl = strImageURL.replaceAll("/../", "");
        String saveStrImageName = currentDate + reStrImageURL;



        System.out.println("reStrImageURL : " +reStrImageURL);
        System.out.println("sliceStrImgUrl : " +sliceStrImgUrl);
        try {
            //open the stream from URL
            URL urlImage = new URL(sliceStrImgUrl);
            URLConnection urlCon = urlImage.openConnection();
            ...
            int n = 0;
            is = urlCon.getInputStream();
            os = new FileOutputStream( IMAGE_DESTINATION_FOLDER + "/" + saveStrImageName);
